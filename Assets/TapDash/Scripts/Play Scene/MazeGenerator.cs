...
public class MazeGenerator : MonoBehaviour
{
    ...
    {
        /* Create object name is "level" to hold paths, diamond, tap point with parent is environment */
        string levelName = "level " + level;
        GameObject level_obj = new GameObject(levelName);
        /* Create object name is "path" to hold "pathTile" objects */
        GameObject path_obj = new GameObject("Path");
        /* Create object name is "tap points" to hold "arrowInactive" objects */
        GameObject tapPoints_obj = new GameObject("Tap Points");
        /* Create object name is "diamonds" to hold "diamond" objects */
        ...
        for (int i = 0; i < lenghtOfPath; i++)
        {
            if (i == 0)
            {
                ...
                GameObject clone = CreateTile(genesisTile, levelsManager[level - 1].startPointOfLevel, Quaternion.identity);
            ...
            {
                GameObject clone = CreateTile(pathTile,
                                        new Vector3(tileLastPosition.x, levelsManager[level - 1].startPointOfLevel.y + sizeOfPathTile - 0.1f, 0.0f),
                                        Quaternion.identity);
                clone.transform.parent = path_obj.transform;
            ...
            /* Create finish tile */
            if (i == lenghtOfPath - 1 && level != levelsManager.Length)
            {
                GameObject clone = CreateTile(finishTile,
                    new Vector3(tileLastPosition.x, tileLastPosition.y + sizeOfPathTile - 0.1f, 0.0f),
                    Quaternion.identity);
                clone.transform.parent = path_obj.transform;
                tileLastPosition = clone.transform.position;
